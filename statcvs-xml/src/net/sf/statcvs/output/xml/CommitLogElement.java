 ...
 */
public class CommitLogElement extends Element {

	private Map revisions = new HashMap();
	
	/**
	 * Renders the commitlog to xml
	 * @param commits 	List of net.sf.statcvs.model.Commit
	 */
	public CommitLogElement(List commits) {
		super("commitlog");
	 ...
	 */
	public static List getCommitList(CvsContent content) {
		RevisionIterator revisionsIt =
			new RevisionSortIterator(content.getRevisionIterator());
		
		// copy and reverse commitlist
		List commits = new ArrayList(new CommitListBuilder(revisionsIt).createCommitList());
	 ...
	 * @return
	 */
	public static List getRecentCommitList(List commits, int maxCommits) {
		return commits.subList(0, maxCommits);
	}
	 ...
	 * @return
	 */
	public static List getUserCommitList(Author author) {
		
		return new CommitListBuilder(author.getRevisionIterator())
			.createCommitList();
	}
}
